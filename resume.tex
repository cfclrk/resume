...
\begin{multicols}{2}
  ...
  \textbf{Languages}: Python, Bash, Clojure, Go

  \textbf{Other}: Kubernetes, Git, Docker, AWS, Azure, Google Cloud Platform,
  Terraform

  \subsection*{Worked with in the Past}

  \textbf{Languages}: Java, Haskell, Javascript, Rust, \LaTeX, C, \CC,
  MATLAB/Octave, CSS/LESS/SASS, SQL

  \textbf{Other}: Vue.js, Gnuplot, Ansible, Packer
...
\begin{itemize}
\item Created automation for deploying Kubernetes clusters (and applications
  running on the clusters)
\item Helped to debug and fix application problems on Kubernetes
\end{itemize}

\tag{Python} \tag{Bash} \tag{Kubernetes}
...
\begin{itemize}
\item Helped design and create an end-to-end test harness for Fugue (mostly
  Python and Bash)
\item Wrote a utility for deleting everything in AWS accounts (Python)
...
\tag{Python} \tag{Bash} \tag{Clojure} \tag{Go} \tag{AWS} \tag{Azure}
...
As a traveling consultant, I traveled to client sites and worked on a variety of
projects that Red Hat was involved with. Mostly, this involved integrating the
JBoss suite of products which Red Hat had recently acquired.

\begin{itemize}
\item \textit{GEICO} -- Helped to build and expand variousd web services. Also
  helped to create an internal error-tracking application using Grails and
  AngularJS.

\item \textit{Cigna} -- Worked on an application for inferring customer
  information using a rules engine.

\item \textit{Kohl's} -- Sole developer in charge of porting functionality from
  the main website to a website for kiosk machines. Also implemented new UI
  features as requested by a UI requirements team.

\end{itemize}

\tag{Java} \tag{JBoss} \tag{jBPM} \tag{Hibernate} \tag{MyBatis} \tag{Grails}
\tag{AngularJS}
...
\item I love all kinds of sports and exercise
\end{itemize}
